...
const SensorModal = (props) => {
  const { buttonLabel, className, type } = props;
  ...
  const handleChange = (event) => {
    ...
    for (var i = 0; i < usersList.length; i++) {
      ...
      if (
        obj.fullName &&
        (obj.fullName.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0 ||
          obj.emailAddress.toLowerCase().indexOf(searchTerm.toLowerCase()) >= 0)
      ) {
        filtered.push(obj);
      }
    }
    ...
    <div>
      ...
      <Modal
        ...
        <ModalHeader toggle={toggle}>
          {type === 'team' ? '+ Create New Team' : '+ Add New Company'}
        </ModalHeader>
        <ModalBody>
          ...
          <div className="teamMemberSearch">
            ...
            />
            <div
              className="teamMemberSearchResults"
              style={{ border: !searchStatus ? 'none' : null }}>
              {!!searchStatus &&
                searchResults.map((item, index) => (
                  <div
                    className="teamMemberResultItem"
                    onClick={() => selectresult(item, index)}
            ...
            <div
              className="teamMemberSearchResults"
              style={{
                border: currentResultItems.length === 0 ? 'none' : null
              }}>
              {!searchStatus &&
                currentResultItems.map((searchresult, index_re) => (
                  <div className="teamMemberResultItem teamMemberResultItem_result">
                    ...
                    <span
                      className="removeBtn"
                      onClick={() => removeResult(searchresult.id)}>
                      ×
                    </span>
                  </div>
                ))}
        ...
        <ModalFooter
          ...
          <button
            ...
            onClick={type === 'team' ? createTeam : createCompany}>
            {type === 'team' ? '+ Create Team' : '+ Add Company'}
          </button>
        </ModalFooter>
      </Modal>
    </div>
...
export default connect(mapStateToProps, mapDispatchToProps)(SensorModal);
